...
PS_BIOS_IMAGES_DIR="${IMG_DIR}/PS/BIOS"
PS2_BIOS_IMAGES_DIR="${IMG_DIR}/PS2/BIOS"
...
function ps_and_ps2_bios_download()
{
    ...
    wget --content-disposition -P "${PS_BIOS_IMAGES_DIR}" "${PS_BIOS_REMOTE}"
    wget --content-disposition -P "${PS2_BIOS_IMAGES_DIR}" \
         "${PS2_BIOS_REMOTE}"
}
...
function emu_land_fetch()
{
    ...
    for URI in $(cat $(dirname "${0}")"/../data/emu-land/nes.list"); do
        wget --content-disposition -P "${NES_GAMES_DIR}" "${URI}"
    done

    for URI in $(cat $(dirname "${0}")"/../data/emu-land/sega_mega_drive.list"); do

        wget --content-disposition -P "${SEGA_MEGA_DRIVE_GAMES_DIR}" "${URI}"
    done

    for URI in $(cat $(dirname "${0}")"/../data/emu-land/snes.list"); do
        wget --content-disposition -P "${SNES_GAMES_DIR}" "${URI}"
    done

    for URI in $(cat $(dirname "${0}")"/../data/emu-land/n64.list"); do
        wget --content-disposition -P "${N64_GAMES_DIR}" "${URI}"
    done

    for URI in $(cat $(dirname "${0}")"/../data/emu-land/zx_spectrum.list"); do
        wget --content-disposition -P "${ZXS_GAMES_DIR}" "${URI}"
    done
}
